...
class Claim(BaseModel):
    claim: str = Field(description="A single factual claim made in the article.")
    paper_ids: List[str] = Field(description="List of paper IDs that support this claim.")
...
class LLMDraftEntry(BaseModel):
    ...
    claims: List[Claim] = Field(
        ...
        description="A list of factual claims extracted from the paragraph, each linked to the paper IDs that support them.",
    )
...
class BlogEntry(BaseModel):
    ...
    source: str = ""
    doi: str = ""
    claims: List[Claim] = Field(default_factory=list)  # Copy from LLMDraftEntry
    verification_status: str = "UNVERIFIED"  # Will be set by main.py after fact-check
...
def _papers_for_prompt(papers: List[Dict], abstract_chars: int) -> str:
    trimmed = [
        {
            "id": p["id"],
            "title": p["title"],
...
def _build_write_task(
    ...
    papers_json = _papers_for_prompt(selected_papers, WRITER_ABSTRACT_CHARS)
    n = len(selected_papers)

    # Base instructions for writing and claims
    description = (
        f"The Reviewer has already chosen the following {n} {category} "
        "papers as this week's most interesting picks.\n\n"
        f"Here are the papers you MUST write about:\n\n"
        f"{papers_json}\n\n"
        ...
        "Additionally, for each article, extract 1-3 concise factual claims "
        "that are directly supported by the abstract. Each claim must be:\n"
        "- factual and independently checkable.\n"
        "- grounded only in the supplied abstract.\n"
        "- associated with one or more of the provided paper IDs that support it.\n"
        "The claims should capture the key assertions of the article that would need verification.\n"
    )

    if feedback:
        description += (
            "\n\nFact-checker feedback from a previous draft:\n"
            f"{feedback}\n\n"
            "Revise the article to address the feedback. Ensure that all claims are fully supported by the abstracts and that every paper ID is correct.\n"
        )

    description += (
        "\n\nExpected output: A JSON object with an 'articles' list containing exactly "
        f"{n} items. Each item must contain: 'id' (exact paper ID), 'title', 'paragraph', "
        "and 'claims' (a list of objects with 'claim' and 'paper_ids'). Do not invent paper IDs."
    )
    ...
    return Task(
        ...
        expected_output=(
            ...
            "items. Each item must contain 'id', 'title', 'paragraph', and 'claims'. "
            "The 'claims' field must be a list of objects, each with 'claim' (string) "
            "and 'paper_ids' (list of strings)."
        ),
        agent=writer,
    ...
    for draft in writer_output.articles:
        ...
        entries.append(
            BlogEntry(
                ...
                doi=paper.get("doi", ""),
                claims=draft.claims,  # Copy claims from draft
                verification_status=verification_status,  # Set by main.py
            )
        )
...
def build_write_crew(
    ...
    write_task = _build_write_task(selected_papers, category, writer, feedback=feedback)
